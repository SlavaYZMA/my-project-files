...
const Camera = () => {
  ...
  // КРИТ-4: Сохранение только через edge function — нет прямых вызовов Storage/DB
  ...
  const downloadVideo = () => {
    ...
    a.href = url;
    a.download = 'eye-recording.webm'; // А-5: без timestamp
    a.click();
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 w-full max-w-2xl pt-20">
        <div className="relative mb-8">
          ...
          <div className="mt-3 text-center">
            <p role="status" aria-live="polite" aria-atomic="true" className={`text-xs transition-colors duration-300 ${
        ...
        {state === 'preview' && !deleteUrl && (
          <div className="flex flex-col gap-4 w-full max-w-xs">
            ...
            <div className="py-3 px-4 border border-yellow-500/20 bg-yellow-500/5">
              <p className="text-yellow-500/70 text-xs leading-relaxed">{t('support.trigger')}</p>
            </div>
            ...
            {/* UI-3: ошибка через state вместо alert() */}
            {saveError && (
              <div role="alert" className="px-4 py-3 border border-red-500/30 bg-red-500/5 text-red-400 text-xs">
                {saveError}
              </div>
            )}
            <button
