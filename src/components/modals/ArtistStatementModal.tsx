...
const ArtistStatementModal = ({ isOpen, onClose }: Props) => {
  ...
  const contentRu = (
    <div className="space-y-6 text-sm leading-relaxed text-white/60">
      ...
      <p>
        Каждый фрагмент — это акт свидетельства без повествования. Анонимность позволяет ему выйти за пределы индивидуальной истории и быть воспринятым как часть общего, коллективного присутствия.
      </p>
      <p>
        Инсталляция существует как постоянно расширяющееся цифровое поле, в котором отдельные взгляды соединяются в непрерывный визуальный поток. Зритель оказывается внутри этого поля, где каждая пара глаз представляет уникальное присутствие, не сводимое к личной истории, статистике или образу жертвы.
      </p>
